...
functor
  ...
  struct
    ...
    (** val p : coq_HypothesisDFA -> Coq_s.string -> int -> Coq_s.string **)

    let p h w i = T.DFA.run (make_dfa h) (Stdlib.List.take i w)

    (** val correct_dec :
      coq_HypothesisDFA -> Coq_s.string -> int -> sumbool **)

    let correct_dec h w i =
      let b = L.member (p h w i @ Stdlib.List.drop i w) in
      if b then
        let b0 = L.member w in
        if b0 then
          Coq_left
        else
          Coq_right
      ...
      (fun fO fS n ->
        ...
        (fun n ->
          ...
          | Some s ->
              ...
              let (Coq_existT (x1, p3)) = s2 in
              let p4, _ = p3 in
              let p5, f1 = p4 in
              let c, _ = p5 in
              lstar_opt n {coq_Q= x1; coq_T= t'; clos= c; fin_Q= f1; fin_T= f}
          | None ->
              Ok (Coq_existT (__, Obj.magic make_dfa h)) )
        fuel
  end
